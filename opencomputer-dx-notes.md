...
- The PR-review demo currently sends a truncated diff as session input instead of checking out the repository into an OpenComputer sandbox/workspace. That undersells OpenComputer for code-review workflows. The ideal API shape would let a managed Durable Agent Session attach to a prepared sandbox/workspace, or accept a first-class GitHub PR source object that securely checks out the repo without exposing installation tokens to model-visible prompt text.
