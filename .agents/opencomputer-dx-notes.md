...
- `@opencomputer/sdk@0.7.2` now exposes session metadata on both `CreateSessionParams` and `SessionData`, so the PR reviewer can use `metadata` without local type casts.
...
- Keep SDK releases tightly synced with docs/API rollouts. During this build, the metadata API/docs were available before the TypeScript declarations, which briefly forced app-side type workarounds.
...
- The user pushed back on using a sandbox as the deployment target. That was valid: Durable Agent Sessions are the app's background-agent backend, but the GitHub App webhook service itself should live on a normal public app host.
- Fly.io was selected as the first public deployment target while the app still had a post-response Node process assumption.
...
- README and deployment docs can drift during iterative prototyping. The repo had an early alternate-host blueprint and provider-specific URL fallback left over after Fly became the target at that checkpoint; docs need an explicit cleanup pass whenever the deployment target changes.
...
- The refactor to `@opencomputer/sdk` removed the local OpenComputer HTTP wrapper cleanly. The SDK already handles auth, retries for safe/idempotent calls, JSON normalization, and typed session handles.
...
- Examples should make the durable boundary explicit: app code may do a short request-scoped handoff to create a session, but the durable background execution and completion notification belong to OpenComputer sessions and destinations/webhooks.
- Host examples should keep the application core runtime-neutral: Fetch/Hono routes, injected config, and no dependency on provider storage primitives unless the example genuinely needs them. That makes Durable Agent Sessions look like the durable backend rather than one piece inside a larger platform-specific stack.
- Once callback routing moved into OpenComputer session `metadata`, the app became a clean serverless fit: Cloudflare Workers can handle only HTTP requests, while OpenComputer carries the durable session state needed to complete and route the review.
- The PR-review demo now tells the managed agent to check out public repositories in its hands sandbox using repo URLs and SHAs, with the diff as fallback context. Private PR checkout is still the harder API stress test: the ideal shape would let a managed Durable Agent Session attach to a prepared sandbox/workspace, or accept a first-class GitHub PR source object that securely checks out the repo without exposing installation tokens to model-visible prompt text.
- For private GitHub repositories, OpenComputer needs a source/credential API that treats checkout privilege as a control-plane or tool-scoped capability. GitHub App installation tokens can be short-lived, repo-scoped, and permission-scoped, but they are still private-repo credentials; they should not be passed through prompts, shell-visible clone URLs, general environment variables, or durable event logs.
...
- `@opencomputer/sdk@0.7.2` now types session metadata directly, removing the local intersection type and `session.snapshot.metadata` cast from the app.
